...
func TestExecute_GithubErr(t *testing.T) {
	...
	When(p.Github.GetModifiedFiles(matchers.AnyModelsRepo(), matchers.AnyModelsPullrequest())).ThenReturn(nil, errors.New("err"))
	r := p.Execute(&planCtx)
...
func TestExecute_CloneErr(t *testing.T) {
	...
	p, _, _ := setupPlanExecutorTest(t)
	When(p.Github.GetModifiedFiles(matchers.AnyModelsRepo(), matchers.AnyModelsPullrequest())).ThenReturn([]string{"file.tf"}, nil)
...
func TestExecute_MultiProjectFailure(t *testing.T) {
	...
	When(p.Github.GetModifiedFiles(matchers.AnyModelsRepo(), matchers.AnyModelsPullrequest())).ThenReturn([]string{"path1/file.tf", "path2/file.tf"}, nil)
	When(p.Workspace.Clone(planCtx.Log, planCtx.BaseRepo, planCtx.HeadRepo, planCtx.Pull, "env")).
		ThenReturn("/tmp/clone-repo", nil)
